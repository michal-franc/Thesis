...
Podstawow¹ funkcj¹ systemu jest mo¿liwoœæ zarz¹dzania kursami zdalnego nauczania wraz
z mo¿liwoœci¹ tworzenia materia³ów , testowania kursantów oraz wystawiania o cen.
\subsubsection{Tworzenie u¿ytkowników oraz przydzielanie uprawnieñ}
System musi posiadaæ mechanizm pozwalaj¹cy zarz¹dzaæ u¿ytkownikami systemu wraz z
mo¿liwoœci¹ przydzielania im odpowiednich poziomów uprawnieñ.
\subsubsection{Zarz¹dzanie kursami}
Ka¿dy Autor oraz Administrator mo¿e stworzyæ jeden lub wiêcej kursów , którymi zarz¹dza.
...
Materia³y nauczania s¹ integraln¹ czêœci¹ kursu. Ka¿dy kurs mo¿e posiadaæ wiêcej ni¿
jeden lub wiêcej materia³ów nauczania. W sk³ad materia³u nauczania wchodz¹
parametry opisowe pozwalaj¹ce sklasyfikowaæ obiekt.
\begin{itemize}
...
W sk³ad obszaru nauczania wchodz¹ sekcje zawieraj¹ce materia³y i wiedz¹ z , której
korzysta kursant. Materia³ nauczania mo¿e posiadaæ jeden lub wiêcej segmentów. Ka¿dy
segment opisany jest tytu³em oraz polem zawartoœci , w którym umieszczamy wiedzê oraz
...
\item Wiêcej informacji -  posiada zbiór dodatkowych materia³ów linków do nich i pozwala
nakierowa³ kursanta je¿eli ten chce poszerzyæ wiedzê wykraczaj¹ca poza zakres materia³u nauczania.
...
kolejne segmenty oraz sekcje wchodz¹ce w sk³ad materia³u nauczania. Na koniec kursant
mo¿e wykonaæ notatki z przeprowadzonego procesu nauczania oraz sprawdziæ swoj¹ wiedzê wykonuj¹c test.
\subsubsection{Zarz¹dzanie testami}
Aplikacja pozwala tworzyæ testy bêd¹ce istotnym elementem procesu nauczania. Testy
mog¹ byæ powi¹zane z kursem lub materia³em nauczania. Kursant posiada mo¿liwoœæ
wyœwietlenia wszystkich testów wchodz¹cych w sk³ad kursu b¹dŸ materia³u nauczania.
...
testów. Test sk³ada siê z segmentu pytania oraz odpowiedzi.Po rozwi¹zaniu
testu wyœwietlana jest ocena oraz wiadomoœæ informuj¹ca o zaliczeniu b¹dŸ nie testu. Nastêpnie ocena zapisywana jest do dzienniczka ucznia
...
Kursant ma mo¿liwoœæ filtrowania kursów na podstawie typu kursu. Mo¿e równie¿ wyœwietliæ listê kursów do , których jest ju¿ zapisany. Kursant ma mo¿liwoœæ filtrowania materia³ów nauczania na podstawie poziomu trudnoœci moce równie¿ sortowaæ materia³y
na podstawie nazwy materia³u.
\subsection{Wymagania niefunkcjonalne}
\subsubsection{£atwoœæ modyfkacji platformy}
...
Platforma komunikuje siê z warstw¹ bazodanow¹ za pomoc¹ us³ug sieciowych. Transmisja danych przebiega³a bêdzie na dwóch poziomach zabezpieczeñ. Pierwszy poziom bez szyfrowania i bez zabezpieczeñ bêdzie s³u¿y³ do przesy³ania danych zawieraj¹cych jedynie dane na temat kursów , testów oraz materia³ów nauczania. Dane przesy³ane na tym poziomie bêd¹ w przesy³ane w formacie JSON by zmniejszyæ iloœæ przesy³anych danych oraz u³atwiæ tworzenie aplikacji po stronie klienta. Dane poufne takie jak dane logowania , has³a bêd¹
przesy³ane bardziej bezpiecznym sposobem transmisji. Komunikacja bêdzie szyfrowana oraz bêdzie implementowa³a mechanizm tokenów pozwalaj¹cy zweryfikowaæ czy nadawca b¹dŸ odbiorca s¹ poprawni i nie dosz³o  do nieuprawnionego przekierowania zapytania b¹dŸ odpowiedzi. Poufne dane 
bêd¹ przesy³ane w formacie XML z zastosowaniem rozwi¹zañ protoko³u SOAP zdefiniowanych w dokumencie WS-Security.
\subsubsection{Komunikacja za pomoc¹ us³ug sieciowych}
Dostêp do bazy danych wystawiony bêdzie w za warstw¹ us³ug sieciowych.
\subsubsection{Mechanizm dynamicznego odnajdywania serwera bazodanowego}
By zwiêkszyæ niezawodnoœæ systemu stworzony zostanie system , który w momencie problemów z komunikacj¹ z podstawowym serwerem bazy danych prze³¹czy system na zapasow¹ bazê danych.
...
Zostanie zagwarantowany dostêp do funkcjonalnoœæi z poziomu wyumienionych wy¿ej przegl¹darek. Nie zostanie zagwarantowany iealnie taki sam 
sposób prezentacji aplikacji.

\subsubsection{Obi¹¿alnoœc}
Aplikacja bêdzie w stanie obs³u¿yæ jednorazowo 100 ¿¹dañ i bêdzie gwarantowaæ czas odpowiedzi w granicach maksymalnie 1 sekundy

\section{Diagramy przep³ywu pracy}
...
Encja s³ownik zawieraj¹ca , Identyfikator typu , klucz do kursu oraz nazwê typu.
\subsubsection{UkonczonyTest}
Encja reprezentuj¹ca ukoñczony test. Zawiera otrzyman¹ ocenê oraz datê ukoñczenia testu. 
\subsubsection{Test}
Reprezentuje Test wype³niany przez kursanta nale¿¹cy do  materia³u nauczania.
\subsubsection{TestType}
S³ownik opisuj¹cy typ Testu
\subsubsection{PytanieTestowe}
Reprezentuje pytanie nale¿¹ce do zbioru pytañ wchodz¹cych w sk³ad testu. Zawiera odpowiedzi , tytu³ oraz treœæ pytania.
...
Reprezentuje wiadomoœæ wyœwietlan¹ w obrêbie ShoutBoxa. Zawiera treœæ , login u¿ytkownika oraz datê przes³ania.
...
Encja ocena wchodz¹ca w sk³ad dziennika. Reprezentuje pojedyñcz¹ ocenê otrzymywan¹ po skoñczeniu testu.
\subsubsection{MaterialNauczania}
Opisuje pojedyñczy materia³ nauczania bêd¹cy podstawowym œrodkiem przekazu informacji dla kursanta.
...
Encja opsiuj¹ca grupê u¿ytkowników. Ka¿dy kurs posiada pojedyñcz¹ grupê u¿ytkowników. 

\subsection{Opis Relacji}

\begin{center}
	\includegraphics{/Relacje/KursGrupa.jpg}
\end{center}

\subsection{Model fizyczny bazy danych}

\begin{center}
	\includegraphics{/Relacje/fizKurs.jpg}
\end{center}
...
\begin{center}
	\includegraphics{/Relacje/allDiagram.jpg}
\end{center}
...
\subsection{Komunikacja z us³ugami sieciowymi}
...
\subsection{Dependancy Injection - NInject}
\subsection{Mockowanie zale¿noœci - RhinoMocks}
\subsection{Mapowanie obiektowo relacyjne - NHibernate}
\subsection{Tworzenie mapowañ - FluentNHibernate}
